...
def tmp_state_dir(tmp_path: Path):
    ...
    with mock.patch("dimos.landmark.landmark_pack.LANDMARK_MEMORY_DIR", tmp_path / "landmark_memory"):
...
def sample_pack_dir(tmp_path: Path):
    ...
    (pack / "landmarks.json").write_text(json.dumps([
        {
            "name": "厨房",
            "record_type": "ROOM",
...
def test_import_pack_clears_session_id(sample_pack_dir: Path, tmp_state_dir: Path, mock_dimos_not_running):
...
def test_import_pack_keep_session_id(sample_pack_dir: Path, tmp_state_dir: Path, mock_dimos_not_running):
...
def test_import_pack_exists_no_force(sample_pack_dir: Path, tmp_state_dir: Path, mock_dimos_not_running):
...
def test_import_pack_exists_with_force(sample_pack_dir: Path, tmp_state_dir: Path, mock_dimos_not_running):
    tmp_state_dir.mkdir(parents=True)
    (tmp_state_dir / "landmarks.json").write_text("[]")
    result = import_pack("test_pack", force=True, pack_dir=sample_pack_dir)
    assert result.record_count == 2
...
def test_round_trip(sample_pack_dir: Path, tmp_state_dir: Path, tmp_path: Path, mock_dimos_not_running):
...
def test_status_with_data(tmp_state_dir: Path):
    tmp_state_dir.mkdir(parents=True)
    (tmp_state_dir / "landmarks.json").write_text(json.dumps([
        {"record_type": "ROOM", "name": "厨房"},
        {"record_type": "LANDMARK", "name": "电脑"},
    ]))
    s = status()
