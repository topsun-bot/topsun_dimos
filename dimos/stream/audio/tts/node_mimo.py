...
"""MiMo TTS 节点, 当 MIMO_API_KEY 可用时替代 OpenAI TTS."""
...
class MiMoVoice:
    ...
    # 中文
    MIMO_DEFAULT = "mimo_default"  # 中国集群默认 冰糖, 其他集群默认 Mia
    BING_TANG = "冰糖"
    MO_LI = "茉莉"
    SU_DA = "苏打"
    BAI_HUA = "白桦"
...
class MiMoTTSNode(AbstractTextConsumer, AbstractTextEmitter):
    """基于 MiMo chat-completions 的 TTS 节点, 接口与 OpenAITTSNode 兼容."""
    ...
    def __init__(
        ...
        self._api_key = api_key or os.environ.get("MIMO_API_KEY") or os.environ.get("OPENAI_API_KEY", "")
        self._model = model
        self._voice = voice
        self._speed = speed
        ...
        if tone_prompt is not None:
            ...
            pace = "slightly fast" if speed > 1.0 else "medium"
            self._tone_prompt = (
                f"Bright, clear, and natural conversational tone — "
                f"like you're speaking casually with a friend. "
                f"{pace} pace, warm and engaging."
            )
    ...
    def consume_text(self, text_observable: Observable[Any]) -> MiMoTTSNode:
        ...
        self._subscription = text_observable.subscribe(
            ...
            on_error=lambda e: logger.error("Error in MiMoTTSNode: %s", e),
        )
        return self
    ...
    def _synthesize_speech(self, text: str) -> None:
        try:
            completion = self._client.chat.completions.create(
                model=self._model,
                messages=[
                    {"role": "user", "content": self._tone_prompt},
            ...
            if not message.audio or not message.audio.data:
                logger.warning("MiMo TTS returned empty audio for: %s", text)
                return
            ...
            try:
                ...
                logger.warning("soundfile 无法解码 MiMo 音频, 跳过")
                return
            ...
            logger.error("MiMo TTS synthesis error: %s", e)

    def dispose(self) -> None:
        logger.info("Disposing MiMoTTSNode")
