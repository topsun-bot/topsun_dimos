        ...
        def _try_navigate() -> str | None:
            ...
            while time.time() < approach_deadline:
                ...
                if abs(dist - _last_logged_dist) > 0.15:
                    logger.info(
                        "Approach '%s': distance=%.2fm (target≤%.2f, accepted≤%.2f) → direct",
                        target.name,
                        dist,
                        arrival_distance,
                        acceptance_distance,
                ...
                if dist <= arrival_distance:
                    break
                # 暂时禁用原来的 1.5m 分段和近距离 inching。中间目标步长小于
                # 规划器自身的到达容差时可能被直接判定到达, 无法保证实际前进。
                # 后续重新设计 inching 前, 远近距离都直接导航到最终参考位姿。
                # if dist > 1.5:
