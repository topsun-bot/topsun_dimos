...
"""CrowAgent — receives natural-language commands from a running CROW
...
class CrowAgent(Module):
    ...
    def receive(self, text: str) -> str:
        ...
        if self._human_transport is None:
            logger.warning("CrowAgent.receive called before start()")
            return "error: CrowAgent not started"

        self._human_transport.publish(text.strip())
        logger.info("CrowAgent → /human_input: %r", text[:120])
