...
class SpatialVectorDB:
    ...
    def _prune_visual_memory_orphans(self) -> None:
        """Drop orphaned frame images without touching room reference images.

        SpatialMemory intentionally shares one VisualMemory between continuous
        ``frame_*`` samples and room snapshots whose IDs are location IDs.  A
        restart may find an empty frame collection while valid room snapshots
        still exist, so pruning every unknown ID would erase room memory.
        """
        retained = set(self._frame_ids)
        for image_id in list(self.visual_memory.images):
            if image_id.startswith("frame_") and image_id not in retained:
