...
class SpatialMemory(Module):
    ...
    def _setup_chromadb(self, *, wipe: bool = False) -> None:
        ...
        self._sync_room_image_order()

    def _sync_room_image_order(self) -> None:
        """Rebuild room-image FIFO state from Chroma and enforce its cap.

        The in-memory list is lost across process restarts, while Chroma and
        VisualMemory persist. Rebuilding it prevents the configured cap from
        counting only images added during the current run.
        """
        try:
            data = self.room_collection.get(include=["metadatas"])
            ids: list[str] = data.get("ids") or []
            metadatas: list[dict[str, Any]] = data.get("metadatas") or [{}] * len(ids)
            ordered = sorted(
                zip(ids, metadatas, strict=False),
                key=lambda pair: float((pair[1] or {}).get("timestamp", 0.0)),
            )
            self._room_image_ids = [image_id for image_id, _ in ordered]
        except Exception:
            logger.warning("Could not rebuild room-image FIFO order", exc_info=True)
            self._room_image_ids = []
            return

        if self.max_room_images <= 0:
            return
        while len(self._room_image_ids) > self.max_room_images:
            ...
            if self._visual_memory is not None:
                self._visual_memory.remove(evict_id)
