...
class GenericAdaptor(AdaptorBase):
    def __init__(self, main_config: Namespace, adaptor_config, state, mlp_config=None):
        ...
        self.use_summary_for_spatial = adaptor_config.get('use_summary_for_spatial', False)
    ...
    def forward(self, input: AdaptorInput) -> RadioOutput:
        ...
        mlp_for_spatial = self.head_mlp if self.use_summary_for_spatial else self.feat_mlp
        feat = mlp_for_spatial(input.features.to(dtype=first_param.dtype), images=input.images, patch_size=input.patch_size).to(dtype=input.features.dtype)
