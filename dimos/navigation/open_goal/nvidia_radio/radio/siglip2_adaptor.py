...
class SigLIP2Adaptor(GenericAdaptor):
    def __init__(self, main_config: Namespace, adaptor_config: dict_t, state: state_t, ckpt_path: str):
        ...
        with rank_gate():
            ...
            siglip2_cache = Path(ckpt_path) / "siglip2"
            model = AutoModel.from_pretrained(version, trust_remote_code=True, cache_dir=siglip2_cache, local_files_only=True)
            proc = AutoProcessor.from_pretrained(version, trust_remote_code=True, cache_dir=siglip2_cache, local_files_only=True)
