...
from .naclip import NAClipAttention
...
class RADIOModel(nn.Module):
    def __init__(
        ...
        naclip_strategy: str = 'naclip',
        naclip_gaussian_std: float = 5.0,
        fixed_patch_dim: Optional[Tuple[int, int]] = None,
        gaussian_device: str = 'cpu',
    ):
        super().__init__()
        ...
        self.naclip_strategy = naclip_strategy
        self.naclip_gaussian_std = naclip_gaussian_std

        if self.use_naclip:
            if not hasattr(self.model, 'blocks') or not isinstance(self.model.blocks[-1], nn.Module):
                raise ValueError("NACLIP requires the last block to be a nn.Module. "
                                 "If you are using a custom model, please ensure it has a compatible last block.")
            if not isinstance(self.model.blocks[-1].attn, Attention):
                raise ValueError("NACLIP requires the last block's attention to be of type Attention.")
            self.addition_cache = dict()

            if fixed_patch_dim is not None:
                self.fixed_patch_dim = fixed_patch_dim

                if self.naclip_strategy in ['naclip', 'nonly']:
                    window_size = [side * 2 - 1 for side in self.fixed_patch_dim]
                    window = NAClipAttention.gaussian_window(*window_size, std=self.naclip_gaussian_std)
                    gaussian_addition = NAClipAttention.get_attention_addition(
                        *self.fixed_patch_dim, window, self.num_summary_tokens, True).unsqueeze(0)
                    self.addition_cache[self.fixed_patch_dim] = gaussian_addition.to(gaussian_device)
                
                last_vit_block = self.model.blocks[-1]
                self.model.blocks[-1].attn = NAClipAttention(
                    orig_attn=last_vit_block.attn,
                    addition_cache=self.addition_cache,
                    num_summary_tokens=self.num_summary_tokens,
                    num_patches=self.fixed_patch_dim,
                    attn_strategy=self.naclip_strategy,
                    gaussian_std=self.naclip_gaussian_std,
                )
            else:
                self.fixed_patch_dim = None
                self.naclip_block = None
    ...
    def naclip(self, x: torch.Tensor, block: nn.Module, num_patches: tuple) -> torch.Tensor:
        """
        Apply NACLIP logic to the last block of the model.
        This function adds a Gaussian kernel to the self-attention mechanism.
        """
        block.attn = NAClipAttention(
    ...
    def forward(self, x: torch.Tensor, feature_fmt: str = 'NLC') -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        if self.use_naclip and not self.fixed_patch_dim:
            vanilla_blocks = self.model.blocks[:-1]
            y = self.model.patch_generator(x)
            y = vanilla_blocks(y)
            y = self.naclip(y, self.model.blocks[-1], num_patches)
