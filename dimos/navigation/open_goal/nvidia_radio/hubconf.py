    ...
    **kwargs,
    ...
    if os.path.isfile(version):
        ...
        chk = load_state_dict_from_url(
            resource.url, progress=progress, map_location="cpu", weights_only=False, model_dir=home_dir / "ckpts"
        )
    ...
    for adaptor_name in adaptor_names:
        ...
        tconf["use_summary_for_spatial"] = kwargs.get('use_summary_for_spatial', False)

        adaptor = adaptor_registry.create_adaptor(ttype, chk["args"], tconf, adaptor_state, ckpt_path=adaptor_ckpt_path)
    ...
    radio = RADIOModel(
        ...
        use_naclip=kwargs.get('use_naclip', False),
        naclip_strategy=kwargs.get('naclip_strategy', 'naclip'),
        naclip_gaussian_std=kwargs.get('naclip_gaussian_std', 5.0),
        fixed_patch_dim=kwargs.get('fixed_patch_dim', None),
        gaussian_device=kwargs.get('gaussian_device', 'cpu'),
    )

    if vitdet_window_size is not None:
        print(f"Applying ViTDet architecture with window size {vitdet_window_size} to the model.")
        apply_vitdet_arch(
            mod,
            VitDetArgs(
                vitdet_window_size,
