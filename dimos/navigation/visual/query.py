...
def _scale_bbox_to_image(
    ...
    if mx <= 1.0:
        ...
        logger.debug(
            "_scale_bbox: fraction %.3f → pixel (%.1f, %.1f, %.1f, %.1f) image=%dx%d",
            mx,
            *result,
            w,
            h,
    ...
    if min(x1, y1, x2, y2) >= 0.0 and mx <= 1000.0:
        ...
        logger.debug(
            "_scale_bbox: Qwen0-1000 (%.1f, %.1f, %.1f, %.1f) → (%.1f, %.1f, %.1f, %.1f) image=%dx%d",
            x1,
            y1,
            x2,
            y2,
    ...
    if 0 <= x1 < x2 <= w + 1 and 0 <= y1 < y2 <= h + 1:
        logger.debug(
            "_scale_bbox: pixel pass-through (%.1f, %.1f, %.1f, %.1f) image=%dx%d",
            x1,
            y1,
            x2,
            y2,
    ...
    if min(x1, y1, x2, y2) >= 0.0:
        ...
        if scale > 0:
            ...
            logger.debug(
                "_scale_bbox: inferred-scale=%.2f (mx=%.1f ref=%.1f) (%.1f, %.1f, %.1f, %.1f) → (%.1f, %.1f, %.1f, %.1f) image=%dx%d",
                scale,
                mx,
                ref,
                x1,
    ...
    logger.debug(
        "_scale_bbox: passthrough (%.1f, %.1f, %.1f, %.1f) image=%dx%d", x1, y1, x2, y2, w, h
    )
    return (x1, y1, x2, y2)
