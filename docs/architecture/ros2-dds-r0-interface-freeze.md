...
Rescued from [#122](https://github.com/topsun-bot/topsun_dimos/pull/122) onto current `main`. Live-tree notes below were re-checked after the #120 upstream merge (zenoh is now the default transport).
...
4. Cyclone [`DomainParticipant`](../../dimos/protocol/service/ddsservice.py) via [`DDSService`](/dimos/protocol/service/ddsservice.py) / `DDSConfig.domain_id` (default **`0`**)
...
- Evidence in-tree: [`dimos/hardware/drive_trains/unitree_go2/adapter.py`](/dimos/hardware/drive_trains/unitree_go2/adapter.py#L130) (`ChannelFactoryInitialize(0)` -- "default domain, default NIC"), [`dimos/robot/unitree/g1/effectors/high_level/dds_sdk.py`](/dimos/robot/unitree/g1/effectors/high_level/dds_sdk.py), [`dimos/hardware/drive_trains/unitree_go2/README.md`](/dimos/hardware/drive_trains/unitree_go2/README.md).
- Do not cite this Cyclone expectation as proof that Chain A Fast-DDS latency is "the" problem.

Default DimOS stream transport is now **zenoh** (`GlobalConfig.transport`, see [`dimos/core/global_config.py`](/dimos/core/global_config.py)). LCM is the legacy opt-in path. Both LCM and zenoh are **out of scope** for this DDS refactor.
...
| Go2 ROS bridge `lidar` | `sensor_msgs/PointCloud2` via `ROSTransport("lidar", PointCloud2)` | Binding frozen in [`unitree_go2_ros.py`](/dimos/robot/unitree/go2/blueprints/smart/unitree_go2_ros.py#L24) |
...
| Zenoh stream transport | **Out of scope.** [`ZenohTransport`](../../dimos/core/transport.py) is the default stream transport on current `main`, not a stub. |

---

## Live-tree drift

R0 cited paths were re-checked against current `main` after #120. Follow the live tree; do not assume the cited nav-image files exist.
...
| Default stream transport | **zenoh** (`GlobalConfig.transport` default). LCM is opt-in. Out of scope. |
| `ZenohTransport` | **Present** as a real LCM-encoded Zenoh transport, not a stub. Out of scope. |
