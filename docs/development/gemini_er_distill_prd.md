...
> 当前推荐路线见：
>
> - [Gemini ER 分层多 Teacher 蒸馏与 G1 单策略方案](/docs/development/gemini_er_multiteacher_distillation_plan.md)
...
| "蒸馏成 72B（或原模型一半）"可行吗？ | 前提不成立：原模型大小未知，且拿不到权重，**"一半"无从算起**。学生模型大小应由部署硬件和开源可得性决定 | 待补 |
...
| 白盒蒸馏（logits/特征对齐）可行吗？ | **不可行**。教师闭源，API 不暴露 logits/中间层 | 待补 |
| 无数据蒸馏（data-free KD）可行吗？ | **不可行**。data-free KD 需要教师白盒（用梯度/BN 统计反向合成输入）。API 教师唯一可行路线是黑盒数据蒸馏 | KD 综述（陶大程 2024 等） |
| 需要采数据吗？ | **需要图像，不需要人工标注**。图像已有（46GB Go2 真值库），标签由教师 API 生成 | 待补 |
