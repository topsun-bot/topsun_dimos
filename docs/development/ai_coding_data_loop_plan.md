...
| 通用 DB 记录器 | `dimos/memory/module.py:318` | `Recorder` 会把所有 `In` port 写入 memory SQLite；replay 模式下会自动禁用记录，避免覆盖原数据。 |
...
4. `ReplayConnection` 用 `TimedSensorReplay("<dataset>/<stream>")` 读取 memory SQLite。
...
3. `Recorder.start()` 为每个输入流建立 memory stream，并把消息 append 到 SQLite。
...
现有 `unitree_go2_memory` 可以作为起点，但它偏向记录 `color_image/lidar/odom`，建议扩展成 `unitree_go2_truth_record` 或增加一个 `RunTruthRecorder`，把导航命令、路径、goal、事件一起存入 memory DB。
