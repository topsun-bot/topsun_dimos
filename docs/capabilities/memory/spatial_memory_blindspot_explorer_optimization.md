...
4. 放入 pending branch queue
5. 选最高优先级 branch 导航过去
6. 到达后识别并等待空间记忆写入
7. 重新 flood fill，更新 branch queue
8. 继续探索下一个 pending branch
```
...
```text
score =
    nearest_distance_m * distance_weight
  - area_m2 * area_weight
  - frontier_bonus
...
→ 记录 arrival_time
→ 等待 get_memory_locations() 中出现 timestamp >= arrival_time 的 frame
...
```python skip
def _wait_for_new_memory_frame(after_timestamp: float, timeout_sec: float) -> bool:
    ...
```
...
memory_frame_confirmed = self._wait_for_new_memory_frame(arrival_time, wait_for_memory_frame_sec)
```
...
4. 从当前 costmap flood fill 找 recovery escape goal
5. 导航到 escape goal，等待短 timeout
6. 如果成功移动出卡住点，重新计算盲区和分支队列
7. 如果连续恢复失败，标记当前 region/branch failed 并停止或换区域
```
