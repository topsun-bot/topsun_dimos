# Unitree Go2 自主探索与栅格地图保存

基于DimOS框架的Unitree Go2机器狗**自主探索与栅格地图保存**系统,使用DimOS内置的导航和地图模块实现真实的自主探索功能,并自动保存栅格地图为ROS标准格式.
...
- **手动保存**: 通过MCP技能随时保存
- **停止保存**: 系统停止时保存最终地图
- **ROS格式**: PGM + YAML(兼容ROS Navigation Stack)
...
- ✅ **智能导航**: A*路径规划 + 动态重规划
- ✅ **前沿检测**: 自动检测未探索区域边界
- ✅ **地图保存**: 自动保存 + 手动保存(PGM+YAML格式)
- ✅ **实时可视化**: Rerun/Foxglove可视化
- ✅ **MCP集成**: 通过MCP协议暴露导航和地图技能,支持LLM控制
