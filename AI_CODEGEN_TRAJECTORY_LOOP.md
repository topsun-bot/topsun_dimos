...
> 围绕 GO2 机器狗"遥控录制 → 数据回灌 → AI 生成代码 → 仿真回放 → 轨迹对比"的完整工程方案。基于对 dimos 仓库现状（`dimos/visualization/rerun`、`dimos/memory`、`dimos/utils/testing/replay.py`、`dimos/robot/unitree/go2`、`dimos/simulation/mujoco`）的实际代码梳理。
...
| "类似 .db / .bag" 的录制方案是否已经存在 | **已存在**。`data/.lfs/*.db` 就是 SQLite 写的"DimOS-bag"，由 `dimos.memory.store.sqlite.SqliteStore` 写入，已经有 `go2_bigoffice.db`、`go2_china_office.db`、`go2_hongkong_office.db`、`go2_short.db`、`go2_slamabuse{1,2}.db` 等真实办公室录制数据。 |
...
| 层 | 文件 | 角色 |
|---|---|---|
| 通用 Store API | `dimos/memory/store/sqlite.py` | `SqliteStore(path, must_exist)`：WAL + sqlite-vec + FTS5 |
| 流式 query | `dimos/memory/stream.py` | `.after / .before / .at / .near / .time_range / .live() / .map / .transform / .search` |
| 大块 payload | `dimos/memory/blobstore/sqlite.py` | LiDAR / 图像放 blob 表，元数据放 observation 表 |
| 编解码 | `dimos/memory/codecs/` | 自动选 `JpegCodec`（图像，10-20× 压缩）/ `LcmCodec`（DimOS 消息）/ `PickleCodec`（兜底） |
| Legacy 写入兜底 | `dimos/utils/testing/legacy_pickle.py` | `LegacyPickleStore`（旧 `TimedSensorStorage` 仍用） |
...
- `dimos/memory/store/README.md` —— memory 设计
...
| 保存运行中的轨迹 | 两个独立通道：(a) Rerun `.rrd`（可视化级），(b) memory `.db`（真值级），见 §4 |
...
```
┌────────────────────────┐   遥控        ┌────────────────────────┐
│ 真车 / WebRTC connect  │ ─────────▶   │ memory SqliteStore    │
...
2. 用 Quest / 手机 / 键盘遥控狗在楼里转，所有 `lidar / odom / color_image / cmd_vel / tf` 自动落到：
   ...
   - 旧的 `TimedSensorStorage` 仍写入 pickle；Truth Capture 新链路直接使用 `Recorder` 与 SQLite DB，旧接口统一迁移仍是后续工作。

3. 结束 Ctrl+C：`.rrd` 自动落盘；`.db` 已增量写完。

### 4.3 真值 schema 约定（建议落地）

为了让 AI 代码评测可重复，约定一条录制必须含以下 streams（topic → memory stream name）：
...
| # | 模块 | 任务 | 优先级 | 落点 |
|---|---|---|---|---|
| 1 | memory 写入 | 统一旧 `TimedSensorStorage` 与新的 `Recorder` SQLite 写入链路 | P0 | `dimos/memory/module.py` + `dimos/utils/testing/replay.py` |
...
| 旧 GO2 录制接口仍写 pickle | 录制工程化弱、扩展性差 | Truth Capture 走 `Recorder` + `SqliteStore`；后续统一旧接口 |
...
- `dimos/utils/testing/replay.py` — `TimedSensorReplay` (memory 适配器) + `timed_playback`
- `dimos/memory/store/README.md` — memory 设计文档
- `dimos/memory/store/sqlite.py` — `.db` 真正的存储后端
...
- **新轨（.db，采用这条）**：memory `Recorder`（`dimos/memory/module.py:318`）→ `SqliteStore`。`Go2Memory`（`dimos/robot/unitree/go2/blueprints/smart/unitree_go2.py:57`）随 `unitree-go2-memory` 蓝图录 `odom/lidar/color_image`；`NavRecord`（`dimos/navigation/nav_stack/modules/nav_record/nav_record.py:37`）可补录 `cmd_vel/path/goal` 等导航流。
